...
<head>
    ...
    <title>MTs Nurul Islam Gunung Toar — Kuis: {{ $materi->judul }}</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .zone-badge{background:linear-gradient(135deg,var(--green-dark),var(--green));color:white;padding:6px 16px;border-radius:20px;font-size:0.78em;font-weight:700;letter-spacing:0.5px;}

        .container{max-width:820px;margin:0 auto;padding:36px 20px;position:relative;z-index:1;}

        .quiz-header{text-align:center;margin-bottom:32px;padding:28px;background:var(--card);border:1.5px solid var(--border);border-radius:18px;box-shadow:0 2px 10px rgba(6,78,59,0.05);}
        .quiz-header h1{font-size:clamp(1.1rem,3vw,1.7rem);font-weight:800;color:var(--green-dark);margin-bottom:8px;}
        .quiz-header p{color:var(--muted);font-size:0.88em;}
        .remedial-badge{display:inline-block;margin-top:12px;background:rgba(217,119,6,0.1);border:1px solid var(--gold);color:var(--gold);padding:5px 16px;border-radius:20px;font-size:0.78em;font-weight:700;}
        ...
        .question-card:focus-within{border-color:var(--green);}

        .q-num{position:absolute;top:-13px;left:18px;background:linear-gradient(135deg,var(--green-dark),var(--green));color:#fff;font-size:0.78em;font-weight:700;padding:4px 14px;border-radius:20px;letter-spacing:0.5px;}
        ...
        .option-label:hover .option-box{border-color:var(--green);background:rgba(5,150,105,0.05);}
        .option-label input:checked ~ .option-box{border-color:var(--green-dark);background:rgba(5,150,105,0.1);box-shadow:0 0 0 3px rgba(5,150,105,0.12);}
        ...
        .option-label input:checked ~ .option-box .opt-letter{background:var(--green-dark);border-color:var(--green-dark);color:white;}

        .submit-area{text-align:center;margin:36px 0 50px;}
        .btn-submit{background:linear-gradient(135deg,var(--green-dark),var(--green));color:#fff;border:none;padding:15px 48px;border-radius:50px;font-family:'Poppins',sans-serif;font-size:0.95em;font-weight:700;cursor:pointer;letter-spacing:0.5px;transition:all .3s;box-shadow:0 6px 20px rgba(6,78,59,0.3);}
        .btn-submit:hover{transform:translateY(-3px);box-shadow:0 10px 28px rgba(6,78,59,0.4);}
        .btn-submit:active{transform:translateY(1px);}
        .done-badge{display:inline-flex;align-items:center;gap:10px;background:rgba(5,150,105,0.08);border:1.5px solid var(--green);color:var(--green-dark);padding:14px 28px;border-radius:50px;font-size:0.88em;font-weight:700;}
...
<body>
    <div class="topbar">
        <a href="{{ route('user.materi.show', $materi->id) }}" class="logo">
        ...
        <div class="topbar-right">
            ...
            <span class="zone-badge"><i class="fas fa-pencil-alt"></i> Zona Kuis</span>
        </div>
    </div>
    ...
    <div class="container">
        <div class="quiz-header">
            ...
            @if(isset($isRemedial) && $isRemedial)
                <div class="remedial-badge"><i class="fas fa-exclamation-triangle"></i> MODE REMEDIAL — Maks. Nilai 72</div>
            @endif
        </div>
            ...
            @if(isset($soals) && is_array($soals) && count($soals) > 0)
                ...
                <div class="submit-area">
                    @if(isset($sudahDikerjakan) && $sudahDikerjakan && !(isset($isRemedial) && $isRemedial))
                        <div class="done-badge"><i class="fas fa-check-circle"></i> KUIS SUDAH DIKERJAKAN</div>
                    @else
                        <button type="submit" class="btn-submit"><i class="fas fa-paper-plane"></i> &nbsp; KUMPULKAN KUIS</button>
                    @endif
                </div>
            @else
                <div class="empty-quiz"><i class="fas fa-clipboard"></i>Belum ada soal untuk kuis ini.</div>
            @endif
        </form>
    </div>
</body>
