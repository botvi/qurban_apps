        ...
        <div class="page-block">
            <div class="row align-items-center">
                ...
                <div class="col-md-12 d-flex align-items-center justify-content-between">
                    <h2 class="mb-0" style="color: #064e3b; font-weight: 800;"><i class="fa-solid fa-file-invoice me-1"></i> Laporan Setoran Tabungan Qurban</h2>
                    <a href="{{ route('reports.deposits', array_merge(request()->all(), ['print' => true])) }}" target="_blank" class="btn btn-warning">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $idx + 1 }}</td>
                                    <td><code>#SET-{{ str_pad($deposit->id, 5, '0', STR_PAD_LEFT) }}</code></td>
                                    <td>{{ date('d/m/Y', strtotime($deposit->tanggal)) }}</td>
                                    <td><strong>{{ $deposit->participant->nama }}</strong></td>
                                    <td>{{ $deposit->keterangan ?? 'Setoran' }}</td>
                                    <td class="text-success font-weight-bold">Rp {{ number_format($deposit->jumlah, 0, ',', '.') }}</td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="6" class="text-center py-4 text-muted">Tidak ada transaksi setoran pada filter ini.</td>
                                </tr>
                                @endforelse
                            </tbody>
                            <tfoot>
                                <tr style="background:#f0fdf4;">
                                    <th colspan="5" class="text-end font-weight-bold" style="color:#064e3b;">TOTAL SETORAN:</th>
                                    <th class="text-success font-weight-bold" style="font-size:1.1em;">Rp {{ number_format($total, 0, ',', '.') }}</th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
